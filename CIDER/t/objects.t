...
isa_ok ($collection_1->type_object, 'CIDER::Schema::Result::Collection',
     'Collection'
 );
is( $collection_1->type, 'collection', 'type is collection.' );

is( $collection_1->collection->language_name, 'English',
...
isa_ok ($series_1->type_object, 'CIDER::Schema::Result::Series',
     'Series'
 );
...
isa_ok ($item_1->type_object, 'CIDER::Schema::Result::Item',
     'Item'
 );
...
is( $item_1->item->date_from, '2000-01-01',
    "Item 1's date_from is correct." );

is( $item_2->item->date_to, '2010-01-01',
    "Item 2's date_to is correct." );

$item_2->item->date_to( '2011-01' );
$item_2->update;

is( $schema->resultset( 'Object' )->find( $item_2->id )->item->date_to,
    '2011-01',
    "Item 2's new date_to is correct." );

# TO DO: re-implement date ranges on collections and series.

# is( $collection_1->collection->date_from, $item_1->item->date_from,
#     "The collection's date_from is the earliest date_from of its subitems.");

# is( $collection_1->collection->date_to, $item_2->item->date_to,
...
my $material = $collection_1->collection->add_to_material( {
    material => 'Test Material 3'
} );
is( $collection_1->collection->material->count, 3,
    'Has 3 associated materials after adding' );
$material->delete;
is( $collection_1->collection->material->count, 2,
    'Has 2 associated materials after deleting' );
is( $collection_1->collection->material->first, 'Test Material 1',
